...
> Documento histórico; não representa sozinho o estado atual do projeto.
...
|---|---|---|
| `frontend/` | Aplicacao web React + Vite + TypeScript | Ja existe painel inicial do Farol consultando views Supabase. Ainda tem heranca do sistema legado. |
...
| `supabase/supabase/` | Backend legado do sistema legado | Referencia local/historica ignorada no Git. Nao misturar com o backend novo sem revisao. |
...
| `Checklist geral privado (XLS-06)` | Regras e validacoes manuais | `CheckList` | 3 |
...
- O caminho `supabase/supabase/` contem historico/rascunhos do sistema legado, migrations antigas e snippets SQL com webhooks/segredos operacionais. Ele foi ignorado no Git para evitar publicacao acidental.
...
2. Renomear metadados do frontend de sistema legado para portal de faturamento.
