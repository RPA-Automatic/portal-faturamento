...
Revisão de ambientes em 2026-09-13: DEV fiscal aguarda projeto exclusivo. Os exemplos `SEU_PROJETO_DEV` abaixo são placeholders e não devem ser usados literalmente. Consulte `supabase/targets.json` antes de registrar callbacks.
...
- Fluxo OAuth do frontend: `implicit`, apropriado para SPA Vite/Netlify sem backend SSR. O retorno esperado usa tokens no hash da URL; o app chama `setSession` e remove os tokens da barra de endereco.
...
```text
https://SEU_PROJETO_DEV.supabase.co/auth/v1/callback
```
...
```text
Homepage URL: https://dev--portal-fiscal-faturamento.netlify.app
Authorization callback URL: https://SEU_PROJETO_DEV.supabase.co/auth/v1/callback
```
...
```text
https://dev--portal-fiscal-faturamento.netlify.app
```
...
```text
Site URL: https://dev--portal-fiscal-faturamento.netlify.app
Redirect URLs:
http://localhost:5173
http://127.0.0.1:5173
https://dev--portal-fiscal-faturamento.netlify.app
