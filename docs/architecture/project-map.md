...
- `frontend/components/Auth.tsx`: tela de login reaproveitada do sistema legado.
- `frontend/package.json`: scripts (`dev`, `build`, `lint`) e dependencias.
- `frontend/types.ts`, `frontend/components/ClientForm.tsx` e componentes relacionados: ainda pertencem ao sistema legado original e podem ser reaproveitados ou removidos aos poucos quando o portal de faturamento amadurecer.
...
- `supabase/supabase/`: historico antigo/local do sistema legado. Esta ignorado no Git; use apenas como referencia privada, sem misturar com o backend novo.
...
- `docs/product/vision.md`: objetivo do produto e público-alvo.
- `docs/architecture/overview.md`: arquitetura do fluxo TOTVS para Farol.
- `docs/architecture/domain-model.md`: entidades principais do domínio.
- `docs/integrations/totvs-data-sources.md`: fontes TOTVS/Datasul e chaves canônicas.
- `docs/data/database.md`: desenho do banco Supabase.
- `docs/product/operational-workflow.md`: etapas E1 a E6 e semáforo.
- `docs/data/xlsx-ingestion-plan.md`: plano prático para transformar XLSX em tabelas do banco.
- `docs/product/roadmap.md`: fases de desenvolvimento.
